package gerenciamento.academia.menus;

import java.io.*;
import java.time.LocalDate;
import java.util.*;

import gerenciamento.academia.modelos.Aluno;
import gerenciamento.academia.modelos.Avaliacao;
import gerenciamento.academia.modelos.Cargo;
import gerenciamento.academia.modelos.Funcionario;
import gerenciamento.academia.modelos.Personal;
import gerenciamento.academia.modelos.Pessoa;
import gerenciamento.academia.modelos.Plano;
import gerenciamento.academia.modelos.PlanoPersonalizado;

public class Cadastro {
	
	static List<Aluno> alunos = new ArrayList<>(); // Lista de alunos
	static List<Personal> personals = new ArrayList<>(); // Lista de personal trainers
	static List<Funcionario> funcionarios = new ArrayList<>(); // Lista de funcionarios
	static List<Avaliacao> avaliacoes = new ArrayList<>(); // Lista de avaliações
	
	static HashSet<PlanoPersonalizado> planosExtras = new HashSet<>();
	
	static HashSet<String> cpfs = new HashSet<>();
	
	public static void main(String[] args) {
		
		String test = "D:/Downloads/pessoas.csv"; //meu diretório pessoal
		cadastrar(test);
		
		System.out.println("\nAlunos: ");
		for(Pessoa p : alunos) {
			System.out.println(p.toString());
		}
		
		System.out.println("\nPersonal Trainers: ");
		for(Pessoa p : personals) {
			System.out.println(p.toString());
		}
		
		System.out.println("\nFuncionários: ");
		for(Pessoa p : funcionarios) {
			System.out.println(p.toString());
		}
		
	}
	
	public static void cadastrar(String arquivoEntrada) {
		
		try(BufferedReader br = new BufferedReader(new FileReader(arquivoEntrada))){
			String linha;
			
			while((linha = br.readLine()) != null) {
				if (linha.isBlank()) continue;
				
				String partes[] = linha.split(",");
				
				String cpf = partes[2].trim().toLowerCase();
				
				if(cpfs.contains(cpf)) {
					System.out.printf("O CPf: %s já foi cadastrado! Ignorado\n", cpf);
					continue;
				}
				else {
					cpfs.add(cpf);
				}
				
				String tipo = partes[0].trim().toLowerCase();
				String nome = partes[1];
				String senha = partes[3].trim().toLowerCase(); //rever
				
				switch(tipo) {
					case "aluno" -> {
						String dataMatricula= partes[4].trim().toLowerCase();
						String plano= partes[5].trim().toUpperCase();
						
						alunos.add(new Aluno(nome, cpf, senha, LocalDate.parse(dataMatricula), Plano.valueOf(plano)));
					}
					case "personal" -> {
						String especialidade = partes[4].trim().toLowerCase();
						String cref = partes[5].trim().toLowerCase();
						
						personals.add(new Personal(nome, cpf, senha, especialidade, cref));
					}	
					default -> { // será funcionário por default
						String cargo = partes[4].trim().toUpperCase();
						
						funcionarios.add(new Funcionario(nome, cpf, senha, Cargo.valueOf(cargo)));	
					}
					
				}	
			}
			
		}catch(IOException e) {
			e.printStackTrace();
			
		}
		
	}
	
}


